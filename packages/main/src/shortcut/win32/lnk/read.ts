...
function readLinkInfo(content, obj) {
    ...
    linkInfo.linkInfoSize = content.readUInt32LE(offset); offset += 4
    linkInfo.linkInfoHeaderSize = content.readUInt32LE(offset); offset +=4;
    linkInfo.linkInfoFlags = content.readUInt32LE(offset); offset +=4
    ...
    linkInfo.volumeIDOffset = content.readUInt32LE(offset); offset +=4
    linkInfo.localBasePathOffset = content.readUInt32LE(offset); offset +=4
    linkInfo.commonNetworkRelativeLinkOffset = content.readUInt32LE(offset); offset +=4
    linkInfo.commonPathSuffixOffset = content.readUInt32LE(offset); offset +=4
    if (linkInfo.linkInfoHeaderSize == 0x24) {
        linkInfo.localBasePathOffsetUnicode = content.readUInt32LE(offset); offset +=4
        linkInfo.commonPathSuffixOffsetUnicode = content.readUInt32LE(offset); offset +=4
    }
    ...
    if (linkInfo.linkInfoFlagBits.hasVolumeIDAndLocalBasePath) {
        ...
        keys.forEach(k => {
            volumnID[k[0].toLowerCase() + k.substr(1)] = content.readUInt32LE(vOffset); vOffset += 4
        })
